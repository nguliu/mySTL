 ...
 *hash_multimap实现
 *
 *hash_multimap使用hashtable作为底层容器，hashtable提供了所有hash_multimap需要的操作
 *hash_multimap使用hashtable的insert_equal来插入元素
 */

#ifndef _STL_HASH_MULTIMAP_H_
#define _STL_HASH_MULTIMAP_H_
...
namespace lfp {
	...
	public:
		hash_multimap() : rep(50, hasher(), key_equal()) { }		//缺省使用表格大小为50
		...
		hash_multimap(InputIterator first, InputIterator last) : rep(50, hasher(), key_equal()) {
	...
	public:
		std::pair<iterator, bool> insert(const value_type& obj) { return rep.insert_equal(obj); }
		template<class InputIterator>
		...
		T& operator[](const key_type& key) {
			return ((*(rep.insert_equal(value_type(key, T())).first)).second);
		}
